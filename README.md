...
### v2.1.0 (Atual) - *Visual Showcase & Deep IA Integration*
- **Vitrine Visual Imersiva**: Implementação de uma galeria de alta fidelidade dentro dos modais de projeto, ilustrando os 4 pilares do produto (Hero, Listagem, Detalhes e Inteligência Artificial).
- **Consolidação de Assets**: Organização estruturada de mídias para os cases de sucesso, garantindo carregamento performático das interfaces.
- **Storytelling Visual**: Cada case agora conta com suporte a imagens reais que validam a complexidade técnica e estética das soluções entregues (foco no ecossistema **Trampo Fácil**).

### v2.0.0 - *The "SaaS Product" Redesign & Real Cases*
- **Transformação Visual**: Migração do conceito de "Landing Page" para "Produto SaaS Interativo". Substituição de seções estáticas por blocos de alta performance visual (Mini-Dashboards, Highligh Blocks).
- **Cases Reais (Trampo Fácil & Trampo IA)**: Inclusão do ecossistema "Trampo Fácil" no portfólio, detalhando a aplicação do motor de IA ("Trampo IA") e o modelo de segurança "Accountless" construído com Supabase e React 19.
- **Processo Interativo**: Conversão da timeline clássica em um *Stepper Horizontal* animado com `framer-motion`, fortalecendo o storytelling visual.
- **Contato Dinâmico**: Remodelagem completa da seção de contato para atuar como um painel de disponibilidade em tempo real, maximizando a taxa de conversão do recrutador/cliente.
- **Micro-Interações Avançadas**: Refinamento das sombras dinâmicas, hover depth em 3D sutil nas tags tecnológicas, e *glow filters* reativos.
